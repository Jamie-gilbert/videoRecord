package com.fooww.videorecord.widgets;
...
import com.fooww.videorecord.bean.VideoModel;
...
public class MyVideoView extends TextureView implements TextureView.SurfaceTextureListener {
     ...
     * @param filePath
     */
    public VideoModel obtainVideoInfo(String filePath) {
        model = new VideoModel(context, filePath);
        if (model.getHeight() < model.getWidth()) {
            getLayoutParams().width = DeviceUtils.getScreenWidth(context);
            getLayoutParams().height = 9 * DeviceUtils.getScreenWidth(context) / 16;
            requestLayout();
        } else {
            reset();
        }
    ...
    public void reset(boolean isLand) {
        if (isLand) {
    ...
    public void start() {
        ...
        if (mMediaPlayer != null && (mCurrentState == STATE_PREPARED || mCurrentState == STATE_PAUSED ||
                mCurrentState == STATE_PLAYING || mCurrentState == STATE_PLAYBACK_COMPLETED)) {
            try {
                if (!isPlaying())
                    mMediaPlayer.start();
    ...
    public void openVideo(Uri uri) {
        ...
        //Prepared 状态：初始化完成之后还需要通过调用prepare()或prepareAsync()
        // 方法，这两个方法一个是同步的一个是异步的，只有进入Prepared状态，才表明MediaPlayer到目前为止都没有错误，可以进行文件播放。
